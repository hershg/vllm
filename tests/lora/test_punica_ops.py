...
@pytest.mark.parametrize("dtype", DTYPES)
@pytest.mark.parametrize("head_shaped_output", [False, True])
@pytest.mark.parametrize(
    "num_heads,kv_lora_rank,qk_nope_head_dim,v_head_dim,lora_rank",
    [(3, 16, 8, 8, 4), (8, 512, 192, 256, 32)],
)
def test_mla_kv_b_lora_uses_explicit_token_mapping(
    dtype,
    head_shaped_output,
    num_heads,
    kv_lora_rank,
    ...
    if head_shaped_output:
        linear_output = linear_output.view(num_tokens, num_heads, full_head_dim)
    expected_linear = linear_output.clone()
    expected_linear_flat = expected_linear.view(num_tokens, -1)
    q_nope = torch.randn(
        num_tokens,
        num_heads,
        qk_nope_head_dim,
    ...
    for token_idx, lora_id in enumerate(mapping.tolist()):
        ...
        a = lora_a[lora_id, 0]
        b = lora_b[lora_id, 0]
        expected_linear_flat[token_idx] = (
            expected_linear_flat[token_idx].float()
