...
class SparseMLACommonImpl(MLACommonBaseImpl[T], Generic[T]):
    ...
    def _compute_context_mha(
        ...
        for chunk in chunked_context.chunks:
            ...
            if request_lora_mapping is not None:
                context_lora_mapping = request_lora_mapping[requests][
                    chunk.token_to_seq.long()
                ]
            k, v = self._project_kv(
                chunk_kv_c, chunk_k_pe, kv_b_proj_lora, context_lora_mapping
            )
    ...
    def forward_mha(  # type: ignore[override]
        ...
        k, v = self._project_kv(kv_c_normed, k_pe, kv_b_proj_lora, token_lora_mapping)
        chunked_context = prefill_metadata.chunked_context
